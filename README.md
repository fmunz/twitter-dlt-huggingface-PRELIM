...
I built this Delta Live Tables (DLT) demo to have a more realistic data engineering end-to end demo using DLT. 
...
Reading a live Twitter stream, ingestion of the streaming data with schema detection, cleansing and transformation of the data, and applying a bit of sentiment analysis to the tweets to classify them as POS, NEG or NEU.
...
What matters in DLT is the "P". "P" for "pipeline". In this example DLT is used together with Databricks Autoloader. Autoloader ingests streaming data into the lakehouse and detects the schema. My DLT pipeline creates a Bronze table for the raw data, then filters the 40 columns per tweet and cleans the data to ensure only tweets in English are contained in the Silver table. Ensuring data quality is done with SQL constraints (we like to call them Expectations in DLT lingo).   
...
alternatively:
* Create a simple Databricks Workflow as described above. This step is easy, but it is not necessary if you want to explore the notebooks manually. 
